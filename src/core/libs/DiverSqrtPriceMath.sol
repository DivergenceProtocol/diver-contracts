...
library DiverSqrtPriceMath {
    ...
    function getNextSqrtPriceFromSpear(uint160 sqrtPrice, uint128 liquidity, uint256 amount, uint unit) internal view returns (uint160 nextSqrtPrice) {
        ...
        // uint256 f1 = b_2l > bs + l ? FullMath.mulDiv(l, b_2-2**96, FixedPoint96.Q96) - bs: FullMath.mulDiv(l, 2**96-b_2, FixedPoint96.Q96) + bs;
        // uint256 f1 = b_2l > bs + l ? FullMath.mulDiv(liquidity, b_2-2**96, 1) - bs: FullMath.mulDiv(liquidity, 2**96-b_2, 1) + bs;
        // uint aaa = liquidity*(b_2-2**96);
        // console2.log("aaa %s", liquidity*(b_2-2**96));
        // // uint bbb = liquidity*(2**96-b_2);
        // console2.log("Q96 %s", uint(2**96));
        // console2.log("b_2 %s", b_2);
        // console2.log("bbb %s", liquidity*(2**96-b_2));
        // uint ccc = aaa - bs;
        // console2.log("ccc");
        // uint ddd = bbb + bs;
        // console2.log("ddd");
        // uint f1 = liquidity*(b_2-2**96) > bs ? liquidity*(b_2-2**96) -bs : liquidity*(2**96-b_2) + bs;
        // uint256 f1 =  FullMath.mulDiv(liquidity, b_2-2**96, 1) > bs ? FullMath.mulDiv(liquidity, b_2-2**96, 1) - bs:
        //              FullMath.mulDiv(liquidity, 2**96-b_2, 1) + bs;

        // uint256 f1 = b_2l > bs + l ? ((sqrtPrice/FixedPoint96.Q96)*(sqrtPrice/FixedPoint96.Q96) - 1) * liquidity - (sqrtPrice/Q96)*amount :
        //  ((1-(sqrtPrice/FixedPoint96.Q96)*(sqrtPrice/FixedPoint96.Q96))*liquidity) + (sqrtPrice/FixedPoint96.Q96)*amount;

        console2.log("f1   %s", f1);
        // if (b_2l > bs) {
        //     f1 = b_2l - bs > l ? b_2l - bs - l : l - (b_2l - bs);
        // } else {
        //     f1 = bs - b_2l + l;
        // }
        uint qd = FixedPoint96.Q96 * unit;
        uint256 f_2 = FullMath.mulDiv(f1, f1 / unit, FixedPoint96.Q96);
        // uint256 f_2 = (f1/qd)*(f1/qd);
        console2.log("f2   %s", f_2);
        // uint256 fourB_2L_2 = 4 * b_2 * l_2;
        uint256 fourB_2L_2 = 4*FullMath.mulDiv(bl, bl, FixedPoint96.Q96*unit);
        // uint256 fourB_2L_2 = 4*FullMath.mulDiv(bl/qd, bl/qd, 1);
        // uint256 fourB_2L_2 = 4*bl*bl;
        
        console2.log("fourB_2L_2    %s", fourB_2L_2);
        uint256 underSqrt = Math.sqrt((f_2 + fourB_2L_2)) * Math.sqrt(FixedPoint96.Q96) * Math.sqrt(unit);
        // uint256 underSqrt = Math.sqrt((f_2 + fourB_2L_2)) * qd;
